"""
Dashboard executif - 100 % registry-driven.
Toutes les valeurs viennent de models/registry.json.
Aucune valeur hardcodee. Change le dataset -> relance le pipeline -> le dashboard s'adapte.
...
.kpi.r { border-left-color:#ef4444; }
...
.kpi .dlt { font-size:13px; font-weight:700; }
.dlt-pos { color:#22c55e; } .dlt-neg { color:#ef4444; }
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## BankRisk AI")
    st.caption("AI Risk & Cost Optimization Engine")
    st.divider()
    st.markdown("**Navigation**")
    st.page_link("app.py",                       label="Tableau de bord Executif")
    st.page_link("pages/1_Fraud_Detection.py",   label="Fraude AML - Detail")
    st.page_link("pages/2_Credit_Risk.py",       label="Risque Credit - Detail")
    st.page_link("pages/3_Cost_Optimization.py", label="Simulateur de Couts")
    st.divider()
    st.caption("Source : `models/registry.json`")
    st.caption("Modele : XGBoost + SMOTE + SHAP")
...
if not summary:
    st.error("Aucun modele entraine. Executez la commande ci-dessous.")
    st.code("python pipelines/run_all.py --fraud-sample 50000", language="bash")
    st.stop()
...
# ══════════════════════════════════════════════════════════════════════════════
# KPIs GLOBAUX
st.markdown('<div class="sec">Impact Business Global</div>', unsafe_allow_html=True)
...
cols[idx].markdown(f"""
<div class="kpi g">
  <div class="lbl">Economie Totale / An</div>
  <div class="val">${total_savings:,.0f}</div>
  <div class="sub">{len(summary)} module(s) actifs</div>
...
for task, info in summary.items():
    ...
    cols[idx].markdown(f"""
    <div class="kpi">
      <div class="lbl">Economie {info['module']}</div>
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION DYNAMIQUE : une section par tache entrainee
for task, info in summary.items():
    lbl    = info.get("labels", {})
    ...
    unit_fp_lbl = lbl.get("unit_fp_label", "cout FP")
    unit_fn_lbl = lbl.get("unit_fn_label", "cout FN")
    vol_lbl     = lbl.get("volume_label",  "volume / an")

    module_name = info.get("module", task.capitalize())
    st.markdown(f'<div class="sec">{module_name} - Ce que le ML change concretement</div>',
                unsafe_allow_html=True)
    ...
    with tab_graph:
        ...
        with c_left:
            ...
            fp_sav  = (fp_leg - fp_ml) * unit_fp if unit_fp else 0
            fn_sav  = (fn_leg - fn_ml) * unit_fn if unit_fn else 0

            fig = go.Figure()
            categories = [unit_fp_lbl, unit_fn_lbl, "Cout total"]
            avant = [fp_leg * unit_fp, fn_leg * unit_fn, legacy_cost]
            apres = [fp_ml  * unit_fp, fn_ml  * unit_fn, ml_cost   ]
            fig.add_trace(go.Bar(
                name="AVANT - Legacy",
                x=categories, y=avant,
                marker_color="#ef4444",
                text=[f"${v:,.0f}" for v in avant], textposition="outside",
            ))
            fig.add_trace(go.Bar(
                name="APRES - ML",
                x=categories, y=apres,
                marker_color="#22c55e",
                text=[f"${v:,.0f}" for v in apres], textposition="outside",
            ))
            fig.update_layout(
                barmode="group",
                title=f"{module_name} - Couts annuels AVANT vs APRES ML",
                yaxis_title="USD / an",
        ...
        with c_right:
            ...
            st.caption(f"Ligne rouge = seuil de qualite ({second_label} > {second_ref})")
    ...
    with tab_table:
        rows = [
            ...
            ("Taux cible positif",         f"{(info.get('fraud_rate',0) or 0):.3%}",
             f"{(info.get('fraud_rate',0) or 0):.3%}", "-"),
            ("-- Modele",                  "Legacy / regles statiques",
             f"XGBoost ({task})", ""),
            ("AUC-ROC",                    "Baseline",
            ...
            ("-- Comptages",               "", "", ""),
            (f"FP / an ({unit_fp_lbl})",   f"{fp_leg:,}",
             f"{fp_ml:,}", f"- {fp_leg-fp_ml:,}"),
            (f"FN / an ({unit_fn_lbl})",   f"{fn_leg:,}",
             f"{fn_ml:,}", f"- {fn_leg-fn_ml:,}"),
            ("-- Couts annuels",           "", "", ""),
            ("Cout total legacy",          f"${legacy_cost:,.0f}", "-", ""),
            ("Cout total ML",              "-", f"${ml_cost:,.0f}", ""),
            ("Economie annuelle",          "-",
             f"${sav:,.0f}", f"ROI {roi} %"),
        ]
        df_t = pd.DataFrame(rows, columns=["Indicateur", "AVANT", "APRES", "Delta"])
        st.dataframe(df_t, hide_index=True, width='stretch', height=430)

    # ── Formules de calcul ───────────────────────────────────────────────────
    with tab_formula:
        st.markdown(f"#### Transparence complete - {module_name}")
        if task == "fraud":
...
<div class="formula">
<b>Cout Legacy</b><br>
  FP = {vol:,} tx &times; (1-{fr:.4f}) &times; {bp:.0%} = <b>{fp_leg:,} alertes</b><br>
  FN = {vol:,} tx &times; {fr:.4f} &times; (1-{br:.0%}) = <b>{fn_leg:,} fraudes manquees</b><br>
  Cout = {fp_leg:,} &times; ${u_fp} + {fn_leg:,} &times; ${u_fn} = <b>${legacy_cost:,.0f}</b>
<br><br>
<b>Cout ML (seuil F1-optimal = {info.get('threshold_opt','?')})</b><br>
  FP extrapole = <b>{fp_ml:,}</b> - Cout = {fp_ml:,} &times; ${u_fp} = ${fp_ml*u_fp:,.0f}<br>
  FN extrapole = <b>{fn_ml:,}</b> - Cout = {fn_ml:,} &times; ${u_fn} = ${fn_ml*u_fn:,.0f}<br>
  Cout total ML = <b>${ml_cost:,.0f}</b>
<br><br>
<b>Economie = ${legacy_cost:,.0f} - ${ml_cost:,.0f} = ${sav:,.0f}  (ROI {roi} %)</b>
</div>""", unsafe_allow_html=True)
        else:
            ...
            st.markdown(f"""
<div class="formula">
<b>Perte Baseline (approuver tout le monde)</b><br>
  Defauts = {n:,} dossiers &times; {dr:.2%} = <b>{fn_leg:,} defauts</b><br>
  Perte = {fn_leg:,} &times; ${loan:,} &times; {lgd:.0%} LGD = <b>${legacy_cost:,.0f}</b>
<br><br>
<b>Cout ML</b><br>
  Defauts approuves (FN) = <b>{fn_ml:,}</b> - Perte = ${fn_ml*loan*lgd:,.0f}<br>
  Bons dossiers rejetes (FP) = <b>{fp_ml:,}</b> - Cout opp. = ${fp_ml*cost.get('cost_per_rejected_good',200):,.0f}<br>
  Cout total ML = <b>${ml_cost:,.0f}</b>
<br><br>
<b>Economie = ${legacy_cost:,.0f} - ${ml_cost:,.0f} = ${sav:,.0f}  (ROI {roi} %)</b>
</div>""", unsafe_allow_html=True)
    ...
    with tab_shap:
        if shap:
            ...
            st.caption("Plus la valeur SHAP est elevee, plus la feature influence la prediction.")
        else:
            st.info("SHAP non disponible. Relancez le pipeline.")

    # ── Simulateur interactif ────────────────────────────────────────────────
    with tab_sim:
        st.markdown(f"#### Simulateur What-If - {module_name}")
        s1, s2 = st.columns([1, 2])

        with s1:
            if task == "fraud":
                sim_vol    = st.number_input("Volume / an", value=int(cost.get("annual_volume", 10_000_000)),
                                              step=1_000_000, format="%d", key=f"vol_{task}")
                sim_ufp    = st.number_input("Cout / fausse alerte ($)", value=int(cost.get("unit_fp", 5)),
                                              step=1, key=f"ufp_{task}")
                sim_ufn    = st.number_input("Perte / fraude manquee ($)", value=int(cost.get("unit_fn", 250)),
                                              step=10, key=f"ufn_{task}")
                sim_fp_rt  = st.slider("Taux FP ML (%)", 0.1, 5.0, 0.8, 0.1, key=f"fpr_{task}") / 100
                sim_rec    = st.slider("Rappel ML (%)", 50, 99, int((info.get("recall") or 0.9)*100),
                ...
                sim_apps   = st.number_input("Dossiers / an", value=int(cost.get("annual_applications", 50_000)),
                                              step=5_000, format="%d", key=f"apps_{task}")
                sim_loan   = st.number_input("Pret moyen ($)", value=int(cost.get("avg_loan_amount", 15_000)),
                                              step=1_000, key=f"loan_{task}")
                sim_lgd    = st.slider("LGD (%)", 20, 70, int(cost.get("loss_given_default", 0.45)*100),
                                        5, key=f"lgd_{task}") / 100
                sim_dr     = st.slider("Taux defaut (%)", 5, 40, int((info.get("fraud_rate") or 0.22)*100),
                                        1, key=f"dr_{task}") / 100
                sim_rec    = st.slider("Rappel defaut ML (%)", 50, 95,
        ...
        with s2:
            ...
            sm1.metric("Legacy / an",  f"${leg_c_s:,.0f}")
            sm2.metric("ML / an",      f"${ml_c_s:,.0f}",   f"-${leg_c_s-ml_c_s:,.0f}")
            sm3.metric("Economie",     f"${sav_s:,.0f}",
            ...
            fn_val_leg = (leg_fn_s * (sim_ufn if task=="fraud" else sim_loan*sim_lgd if task!="fraud" else 0)) if task=="fraud" else leg_c_s - fp_val_leg
            ...
            fig_s.add_trace(go.Bar(name="Legacy", x=cats_s,
                                    y=[fp_val_leg, fn_val_leg, leg_c_s],
                                    marker_color="#ef4444",
                                    text=[f"${v:,.0f}" for v in [fp_val_leg, fn_val_leg, leg_c_s]],
                                    textposition="outside"))
            fig_s.add_trace(go.Bar(name="ML", x=cats_s,
                                    y=[fp_val_ml, fn_val_ml, ml_c_s],
                                    marker_color="#22c55e",
...
# ══════════════════════════════════════════════════════════════════════════════
# WATERFALL GLOBAL
st.markdown('<div class="sec">Decomposition des Economies - Vue Globale</div>',
            unsafe_allow_html=True)

wf_x, wf_y, wf_m, wf_t = ["Cout total legacy"], [total_legacy], ["absolute"], [f"${total_legacy:,.0f}"]
...
for task, info in summary.items():
    ...
    fp_sav_ = (fp_leg_ - fp_ml_) * u_fp_
    fn_sav_ = (fn_leg_ - fn_ml_) * u_fn_
...
fig_wf.update_layout(
    title=f"D'ou vient chaque dollar economise ? - Total : ${total_savings:,.0f} / an",
...
# ── Historique des runs ───────────────────────────────────────────────────────
st.markdown('<div class="sec">Historique des Entrainements</div>', unsafe_allow_html=True)
...
<div class="footer">
  ...
  <strong>Realise par Oswald Jaures KOFFI</strong>
</div>
""", unsafe_allow_html=True)
