...
tab1, tab2, tab3, tab4 = st.tabs([
    "Performance", "Threshold Tuning", "SHAP Explainability", "Cost Analysis"
])
...
with tab1:
    ...
    col4.metric("Recall",    m_opt["recall"],    help="Fraud cases caught")
    col5.metric("Precision", m_opt["precision"], help="Of flagged: truly fraud")
...
with tab2:
    ...
    st.info("Compare precision vs recall trade-off across stored threshold configurations.")
    ...
    st.markdown("""
    **Key trade-off:**
    - Higher threshold → fewer alerts, higher precision, lower recall (more fraud missed)
    - Lower threshold → more alerts, lower precision, higher recall (more manual review)
    """)
...
with tab3:
    ...
    if not feat_df.empty and "mean_shap" in feat_df.columns and "feature" in feat_df.columns:
        ...
        st.markdown("""
        **Interpretation:**
        - **V14, V12, V10** : Most discriminant PCA components for fraud detection
        - **Amount_log** : Transaction amount is a strong signal
        - **amount_hour_zscore** : Unusual amounts at a given hour
        """)
    else:
        st.info("SHAP not available - re-run the fraud pipeline.")
...
with tab4:
    ...
    values = [
        ...
        -(s["legacy_fp_alerts"] - s["ml_fp_alerts"]) * 5,
        -(s["legacy_fn_missed"] - s["ml_fn_missed"]) * 250,
        s["ml_annual_cost"],
    ]
